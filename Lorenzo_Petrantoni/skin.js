...
// Pano2VR 6.0.1/17227
// Filename: 360.ggsk
// Generated lun feb 16 14:13:34 2026

function pano2vrSkin(player,base) {
	player.addVariable('set_text', 0, "");
	var me=this;
	var skin=this;
	var flag=false;
	var nodeMarker=[];
	...
	this.addSkin=function() {
		...
		el=me._black_bg=document.createElement('div');
		el.ggId="BLACK_BG";
		el.ggDx=0;
		el.ggDy=0;
		el.ggParameter={ rx:0,ry:0,a:0,sx:1,sy:1 };
		...
		el.setAttribute('style',hs);
		el.style[domTransform + 'Origin']='50% 50%';
		me._black_bg.ggIsActive=function() {
			return false;
		}
		el.ggElementNodeId=function() {
			return player.getCurrentNode();
		}
		me._black_bg.ggUpdatePosition=function (useTransition) {
		...
		me.divSkin.appendChild(me._black_bg);
		el=me._welcome_page=document.createElement('div');
		els=me._welcome_page__img=document.createElement('img');
		els.className='ggskin ggskin_welcome_page';
		hs=basePath + 'images/welcome_page.png';
		els.setAttribute('src',hs);
		els.ggNormalSrc=hs;
		els.setAttribute('style','position: absolute;top: 0px;left: 0px;width: 100%;height: 100%;-webkit-user-drag:none;pointer-events:none;;');
		els.className='ggskin ggskin_image';
		...
		el.setAttribute('style',hs);
		el.style[domTransform + 'Origin']='50% 50%';
		me._welcome_page.ggIsActive=function() {
			return false;
		}
		el.ggElementNodeId=function() {
			return player.getCurrentNode();
		}
		me._welcome_page.logicBlock_size = function() {
			...
			if (me._welcome_page.ggCurrentLogicStateSize != newLogicStateSize) {
				me._welcome_page.ggCurrentLogicStateSize = newLogicStateSize;
				me._welcome_page.style[domTransition]='width 0s, height 0s';
				if (me._welcome_page.ggCurrentLogicStateSize == 0) {
					me._welcome_page.style.width='960px';
					me._welcome_page.style.height='540px';
					skin.updateSize(me._welcome_page);
				}
				else {
					me._welcome_page.style.width='1920px';
					me._welcome_page.style.height='1080px';
					skin.updateSize(me._welcome_page);
				}
			}
		}
		me._welcome_page.onclick=function (e) {
			player.activateSound("_background","1");
			me._welcome_page.style[domTransition]='none';
			me._welcome_page.style.opacity='0';
			me._welcome_page.style.visibility='hidden';
			player.playSound("_background","0");
			me._welcome_page.style[domTransition]='none';
			me._welcome_page.style.visibility='hidden';
			me._welcome_page.ggVisible=false;
			me._black_bg.style[domTransition]='none';
			me._black_bg.style.visibility='hidden';
			me._black_bg.ggVisible=false;
			me._mouse_block.style[domTransition]='none';
			me._mouse_block.style.visibility=(Number(me._mouse_block.style.opacity)>0||!me._mouse_block.style.opacity)?'inherit':'hidden';
			me._mouse_block.ggVisible=true;
			player.openNext("{node26}","");
			player.startAnimation("flyin");
		}
		me._welcome_page.ggUpdatePosition=function (useTransition) {
			if (useTransition==='undefined') {
				useTransition = false;
			}
			if (!useTransition) {
		...
		me.divSkin.appendChild(me._welcome_page);
		el=me._mouse_block=document.createElement('div');
		el.ggId="mouse_block";
		el.ggDx=0;
		el.ggDy=0;
		el.ggParameter={ rx:0,ry:0,a:0,sx:1,sy:1 };
		...
		el.setAttribute('style',hs);
		el.style[domTransform + 'Origin']='50% 50%';
		me._mouse_block.ggIsActive=function() {
			return false;
		}
		el.ggElementNodeId=function() {
			return player.getCurrentNode();
		}
		me._mouse_block.ggUpdatePosition=function (useTransition) {
			if (useTransition==='undefined') {
				useTransition = false;
			}
			if (!useTransition) {
		...
		me.divSkin.appendChild(me._mouse_block);
		...
		player.addListener('varchanged_set_text', function() {if (	(		(player.getVariableValue('set_text') == "end1")	)) {	player.stopAutorotate();}if (	(		(player.getVariableValue('set_text') == "end1")	)) {	player.openNext("{node15}","");}if (	(		(player.getVariableValue('set_text') == "end1")	)) {	me._mouse_block.style[domTransition]='none';	me._mouse_block.style.visibility='hidden';	me._mouse_block.ggVisible=false;}});
	};
...
player.addListener('timer', me._marker_active.ggUpdateConditionTimer);
	...
	me._welcome_page.logicBlock_size();
	player.addListener('sizechanged', function(args) { me._web.logicBlock_scaling(); });
	player.addListener('configloaded', function(args) { me._marker_title.logicBlock_position();me._web.logicBlock_scaling();me._welcome_page.logicBlock_size(); });
	me.skinTimerEvent();
};